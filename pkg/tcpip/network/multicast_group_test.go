...
func createStack(t *testing.T, v4 bool, mgpEnabled bool) (*channel.Endpoint, *stack.Stack, *faketime.ManualClock) {
	t.Helper()

	var igmpEnabled bool
	var mldEnabled bool
	if mgpEnabled {
		if v4 {
			igmpEnabled = true
...
func TestMGPDisabled(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			e, s, clock := createStack(t, test.protoNum == ipv4.ProtocolNumber, false)
...
func TestMGPReceiveCounters(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			e, s, _ := createStack(t, len(test.groupAddress) == header.IPv4AddressSize, true)
...
func TestMGPJoinGroup(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			e, s, clock := createStack(t, test.protoNum == ipv4.ProtocolNumber, true)
			...
			for _, subTest := range subTests {
				t.Run(subTest.name, func(t *testing.T) {
					...
					for i := 0; i < maxUnsolicitedReports; i++ {
						...
						reportCounter++
						if got := sentReportStat.Value(); got != reportCounter {
							t.Errorf("(i=%d) got sentReportState.Value() = %d, want = %d", i, got, reportCounter)
						}
						if p, ok := e.Read(); !ok {
